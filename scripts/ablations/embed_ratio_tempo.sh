...
EXPERIMENT="$ARGS_CMD/mgcom_tempo_executor.py --experiment=abl_emb_ratio_tempo $ARGS_BASE --lr=0.02 --repr_dim=32 --num_workers=3 --metric=DOTP"
...
#  $(echo $EXPERIMENT) --run_name="embed0.02" $(echo "$ARGS_DS $EMBED_DS") --embed_node_ratio=0.02
#  $(echo $EXPERIMENT) --run_name="embed0.05" $(echo "$ARGS_DS $EMBED_DS") --embed_node_ratio=0.05
#  $(echo $EXPERIMENT) --run_name="embed0.07" $(echo "$ARGS_DS $EMBED_DS") --embed_node_ratio=0.07
