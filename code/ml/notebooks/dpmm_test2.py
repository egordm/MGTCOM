...
def plot_results(X, z, mus, covs, title, colors):
...
class PlotCallback(EMCallback):
    ...
    def on_after_init_params(self, model: DirichletProcessMixture) -> None:
        super().on_after_init_params(model)
        self.run += 1
        self.step = 0

    def on_after_step(self, model: DirichletProcessMixture, lower_bound: Tensor) -> None:
        fig = plot_results(
            X,
            model.predict(X),
            model.mus,
            model.covs,
            f"Run {self.run}; Step {self.step}",
