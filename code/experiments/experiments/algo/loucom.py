...
class CentroidCounter:
    ...
    @staticmethod
    def from_centroids(centroids: Tensor) -> 'CentroidCounter':
        return CentroidCounter(centroids, torch.ones(len(centroids), dtype=torch.long))

    def merge(self, from_idx: int, to_idx: int):
        if from_idx == to_idx:
            return

        self.centroids[to_idx] = (
                self.centroids[from_idx, :] * self.counts[from_idx] +
                self.centroids[to_idx, :] * self.counts[to_idx]
        ) / (self.counts[from_idx] + self.counts[to_idx])
        self.counts[to_idx] += self.counts[from_idx]
        self.counts[from_idx] = 0

    def aggregate(self, n_comms: int, index: Tensor) -> 'CentroidCounter':
        counts = torch.zeros(n_comms, dtype=torch.long)\
            .index_add(0, index, self.counts).maximum(torch.tensor(1, dtype=torch.long))
        centroids = torch.zeros(n_comms, self.rep_dim, dtype=torch.float)\
            .index_add(0, index, self.centroids * self.counts.unsqueeze(1))
        return CentroidCounter(centroids / counts.unsqueeze(1), counts)

    def clone(self):
        return CentroidCounter(self.centroids.clone(), self.counts.clone())
...
class CooGraph:
    ...
    @staticmethod
    def from_edgelist(n_nodes, centroids: CentroidCounter, edge_index: Tensor, weights: Tensor) -> 'CooGraph':
        ...
        return CooGraph(n_nodes, centroids, edge_index, weights, edge_index_rev, perm_rev)
...
class MultilevelCommunityList:
    ...
    centroids: CentroidCounter  # Centroids of the communities
...
def multilevel_community_links(
    ...
    sims = pairwise_dotp(graph.centroids[vertex].unsqueeze(0), comms.centroids[neighbors])
    weight = graph.weights[neigh_edge_ids] * sims
    to = neighbors
...
def community_multilevel_step(
    ...
    comms = MultilevelCommunityList(
        ...
        centroids=graph.centroids.clone(),
        communities=[
    ...
    for ((ffrom, fto), w) in zip(graph.edge_index.t(), graph.weights):  # TODO: Can be sped up
        sim = torch.dot(graph.centroids[ffrom], graph.centroids[fto])
        comms.communities[ffrom].weight_all += w * sim
        comms.communities[fto].weight_all += w * sim
        weight_sum += w * sim
        if ffrom == fto:
            comms.communities[ffrom].weight_inside += 2 * w * sim

    comms.weight_sum = 2 * weight_sum
    ...
    while True:  # Pass begin
        ...
        tmp_comm = comms.n_comms, comms.membership.clone(), comms.centroids.clone()
        ...
        for i in range(0, n_nodes):
            ...
            comms.centroids.merge(old_id, new_id)
        ...
        if changed and q > pass_q:
            ...
            comms.n_comms, comms.membership, comms.comm_centroids = tmp_comm
            break
    ...
    centroids = graph.centroids.aggregate(n_nodes, membership)
    ...
    new_graph = CooGraph.from_edgelist(n_nodes, centroids, edge_index, weights)
...
def loucom(
    ...
    graph = CooGraph.from_edgelist(n_count, CentroidCounter.from_centroids(centroids), edge_index, weights)
    ...
    while True:
        ...
        cluster_centroids.append(step_graph.centroids.centroids)
        cluster_counts.append(step_graph.n_nodes)
    ...
    return level_memberships, modularity, cluster_counts, cluster_centroids
