...
class WeightedNeighborLoader:
    ...
    def __call__(self, indices: List[int]):
        ...
        sample_fn = thg.algo.neighbor_sampling_homogenous_weighted
        nodes, rows, cols, edge_index, layer_offsets = sample_fn(
            self.col_ptrs, self.row_indices, self.weights, index, self.num_neighbors
        )
