...
from ml.algo.dpmm.base import BaseMixture, P
from ml.algo.dpmm.dpm import DPMMParams, DirichletProcessMixture, DPMixtureParams
...
from ml.utils import Metric, unique_count, mask_from_idx
...
class DPMMSCParams(NamedTuple):
    cluster: DPMMParams
    subcluster: List[DPMMParams]


@dataclass
class DPMixtureSCParams(DPMixtureParams, MHParams):
    mutate: bool = True


# noinspection PyProtectedMember
class DirichletProcessMixtureSC(BaseMixture[DPMMSCParams]):
    hparams: DPMixtureSCParams

    def __init__(self, hparams: DPMixtureSCParams) -> None:
    ...
    def _create_subcluster(self) -> DirichletProcessMixture:
        ret = DirichletProcessMixture(
            DPMixtureSCParams(**{
                **self.hparams.to_dict(),
                'init_k': 2,
                'init_mode': InitMode.KMEANS1D,
            })
        )
    ...
    @property
    def mus(self) -> Tensor:
        return self.clusters.mus

    @property
    def covs(self) -> Tensor:
        return self.clusters.covs

    @property
    def mus_sub(self) -> List[Tensor]:
        return [cluster.mus for cluster in self.subclusters]

    @property
    def covs_sub(self) -> List[Tensor]:
        return [cluster.covs for cluster in self.subclusters]
    ...
    def _mutate_split(
        ...
        self.reinit_count = [0 for i in range(self.n_components)]
