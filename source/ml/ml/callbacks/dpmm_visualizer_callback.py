...
class DPMMVisualizerCallback(Callback):
    ...
    def setup(self, trainer: Trainer, pl_module: DPMMSubClusteringModel, stage: Optional[str] = None) -> None:
    ...
    def on_validation_epoch_end(self, trainer: Trainer, pl_module: DPMMSubClusteringModel) -> None:
        ...
        visualize_subclusters = pl_module.hparams.subcluster and pl_module.subclusters.is_initialized

        logger.info(f'Visualizing decision boundaries at epoch {trainer.current_epoch}')
        X_t = self.X_t
        r = pl_module.validation_outputs['r']
        I = self.subsampler.transform(r.argmax(dim=-1))

        if visualize_subclusters:
            ri = pl_module.validation_outputs['ri']
            sub_I = self.subsampler.transform(ri.argmax(dim=-1))
        else:
            ri, sub_I = None, None

        # Extract Cluster parameters
        mus = pl_module.clusters.mus.detach().cpu()
        covs = pl_module.clusters.covs.detach().cpu()
        k = pl_module.clusters.n_components

        if visualize_subclusters:
            sub_mus = pl_module.subclusters.mus.detach().cpu()
            sub_covs = pl_module.subclusters.covs.detach().cpu()
        else:
    ...
    def plot_clusters(
        ...
        plot_scatter(
            ...
            markers=['+', 'x'], marker_idx=sub_I,
            linewidth=2, s=self.marker_size * 4, zorder=3
        )
        ...
        plot_scatter(
            ...
            label="Net Centers",
            s=self.marker_size * 8, linewidth=2,
            alpha=0.6, zorder=4
        )
        draw_ellipses(ax, mus, covs, colors, alpha=0.4, zorder=1)

        # Draw subclusters
        if sub_covs is not None:
            indices = (torch.arange(len(sub_mus)) / 2).floor().long()

            draw_ellipses(ax, sub_mus, sub_covs, colors[indices], alpha=0.4, zorder=2)
            plot_scatter(
    ...
    def plot_decision_regions(self, ax, X_t, I, colors, pl_module: DPMMSubClusteringModel):
