...
class HeteroNodeEmbedding(torch.nn.Module):
    def __init__(self, num_nodes_dict: Dict[NodeType, int], repr_dim: Union[int, Dict[NodeType, int]]):
        super().__init__()
        self.num_nodes_dict = num_nodes_dict
        self.repr_dim = repr_dim if isinstance(repr_dim, dict) \
            else {node_type: repr_dim for node_type in num_nodes_dict}

        self.embedding_dict = torch.nn.ModuleDict({
            node_type: NodeEmbedding(num_nodes, self.repr_dim[node_type])
