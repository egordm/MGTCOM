...
ARGS_ICEWS="--batch_size=128 --max_epochs=60 --dataset=ICEWS0515 --embedding_visualizer.dim_reduction_mode=TSNE --embedding_visualizer.interval=10 --classification_eval.interval=10"
...
$(echo $EXPERIMENT) --run_name="unbiased" $(echo "$ARGS_DS $EMBED_DS") --use_unbiased=true
$(echo $EXPERIMENT) --run_name="ballroom" $(echo "$ARGS_DS $EMBED_DS") --use_unbiased=false
