...
# Tune E2E clustering
EXPERIMENT="$ARGS_CMD/mgcom_e2e_executor.py --experiment=mgtcome2e_tume_metric $ARGS_BASE"
$(echo $EXPERIMENT) --prior_sigma_scale=0.05 --init_k=10 --use_tempo=false --run_name="mL2" --metric=DOTP $(echo $ARGS_DS)
$(echo $EXPERIMENT) --prior_sigma_scale=0.05 --init_k=10 --use_tempo=false --run_name="mDOTP" --metric=L2 $(echo $ARGS_DS)
